...
## POC Product UI Overrides

These overrides apply to the reconciliation agent POC and should be used when stitching additional authenticated product flows into this workspace.

- Do not use all-caps headings, eyebrow text, labels, or status text. Use sentence case or title case.
- Use lighter type weights for product UI. Prefer 400 for body, 500 for headings and labels, and 600 only for primary emphasis.
- Keep button padding compact: standard buttons use 8px horizontal padding; micro buttons, chips, and pills use 6px horizontal padding.
- The center workspace is for active work only. Do not duplicate observability or timeline streams there.
- The right rail is the persistent observability surface. It owns context, input inventory, current work, and run timeline.
- Keep copy short. Agent actions should describe the current action, not explain the product.
- Use lucide-react icons for actions and status affordances.
- Avoid nested cards. Bank files should read as compact blocks, not as tables.
